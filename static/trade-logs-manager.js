...
console.log('[TradeLogsManager] script loaded');

class TradeLogsManager {
    constructor() {
        // DIAG: экземпляр создаётся
        console.log('[TradeLogsManager] constructor');
     ...
     */
    createToggleButton() {
        // Попробуем найти заголовок по разным селекторам, чтобы быть устойчивыми к структуре шаблона
        let breakevenHeader = document.querySelector('.breakeven-table h3')
            || document.querySelector('.breakeven-table .params-header h3')
            || document.querySelector('.card.breakeven-table h3');

        // Если заголовок не найден, попробуем создать его внутри .params-header или внутри .breakeven-table
        if (!breakevenHeader) {
            const paramsHeader = document.querySelector('.breakeven-table .params-header') || document.querySelector('.breakeven-table');
            if (paramsHeader) {
                ...
                h3.style.gap = '10px';
                h3.innerHTML = `<span>📊 Таблица безубыточности</span>`;
                paramsHeader.insertBefore(h3, paramsHeader.firstChild);
                breakevenHeader = h3;
            }
        }

        if (!breakevenHeader) {
            console.warn('[TradeLogsManager] Не удалось найти или создать заголовок таблицы безубыточности');
            return;
        }

        // Вставляем кнопки-переключатели внутрь найденного заголовка
        const currentTitle = breakevenHeader.querySelector('span') ? breakevenHeader.querySelector('span').textContent : 'Таблица безубыточности';
        breakevenHeader.innerHTML = `\
            <div class="view-toggle-group">\
                <button id="btn-view-breakeven" class="view-toggle-btn active">${this.escapeHtml(currentTitle)}</button>\
...
if (document.readyState === 'loading') {
    ...
    try { window.tradeLogsManager.init(); } catch (e) { console.error('TradeLogsManager init error', e); }
            ...
            this.loadLogs(window.currentBaseCurrency || null);
