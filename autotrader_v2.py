        ...
        try:
            ...
            try:
                ...
                with lock:
                    ...
                    # 🔴 КРИТИЧЕСКИ ВАЖНО: Таблица ВСЕГДА считается от ПЕРВОЙ покупки (P0)!
                    # Средневзвешенная цена НЕ используется для расчёта таблицы безубыточности!
                    # Таблица определяет ОТНОСИТЕЛЬНЫЕ падения от P0, а не абсолютные цены!
                    
                    # Пересчитываем таблицу с ПЕРВОНАЧАЛЬНОЙ ценой покупки (P0)
                    new_table = calculate_breakeven_table(params, current_price=cycle.start_price)
                    cycle.table = new_table
                    
                    # Для информации: рассчитываем средневзвешенную цену
                    if cycle.base_volume > 0:
                        weighted_avg_price = cycle.total_invested_usd / cycle.base_volume
                        print(f"[{base}] [INFO] Средневзвешенная цена позиции: {weighted_avg_price:.8f}")
                        print(f"[{base}] [INFO] Цена первой покупки (P0): {cycle.start_price:.8f}")
                    else:
