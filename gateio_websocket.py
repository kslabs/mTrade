...
class GateIOWebSocket:
    ...
    def connect(self):
        ...
        try:
            ...
            logger.info(f"WebSocket connect() вызван для {self.ws_url}")

            time.sleep(1)

            logger.info(f"После sleep(1): is_running={self.is_running}, error={self.error}")
...
class PairWebSocketManager:
    ...
    def create_connection(self, currency_pair: str) -> GateIOWebSocket:
        ...
        with self.lock:
            ...
            while not ws_client.is_running and time.time() - start_time < 5:
            ...
            if not ws_client.is_running:
                ...
                logger.error(f"WebSocket не подключился для {pair_formatted} в течение 5 секунд")

                # Продолжаем, чтобы подписки могли быть отправлены позже
            ...
            def ticker_callback(data):
                ...
                with self.lock:
                    ...
                    if isinstance(data, dict) and 'currency_pair' in data:
                        ...
                        # Лог обновления тикера убран (засоряет логи)
                ...
                with self.lock:
                    ...
                    if isinstance(data, dict):
                        ...
                        if 'asks' in data and 'bids' in data:
                            ...
                            # Лог обновления стакана убран (засоряет логи)
